...
export async function buildArtistSession(
  token: string,
  artistName: string,
  count = 5,
): Promise<string> {
  const [artistTracks, likedTracks] = await Promise.all([
    getArtistTracks(artistName, token),
    getLikedTracks(token),
  ])
  const playable = artistTracks.filter((t) => t.preview_url !== null)

  if (playable.length < count) throw new NotEnoughTracksError()

  const selected = [...playable].sort(() => Math.random() - 0.5).slice(0, count)

  const lower = artistName.toLowerCase()
  const distractorPool = likedTracks.filter(
    (t) => t.artists[0]?.name.toLowerCase() !== lower,
  )
  ...
  const clips: Clip[] = selected.map((track) => ({
    ...
    albumImageUrl: track.album.images[0]?.url ?? null,
    songQuestion: buildSongQuestion(track, playable),
    artistQuestion: buildArtistQuestion(track, distractorPool),
  }))
